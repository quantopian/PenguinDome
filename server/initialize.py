...
def main(args):
    ...
    if do_service and not args.skip_systemctl:
    ...
    if service_exists and not args.skip_systemctl:
        try:
    ...
    # There are syntactically unnecessary double parentheses below to make
    # flake8 happy.
    if not args.skip_build_release and \
       ((client_changed or
         not glob.glob(os.path.join(releases_dir, '*.tar.asc')))):
        if client_changed:
...
def parse_args():
    ...
    parser.add_argument('--skip-systemctl', action='store_true',
                        help='Do not use systemctl')
    parser.add_argument('--skip-build-release', action='store_true',
                        help='Do not build a client release')
